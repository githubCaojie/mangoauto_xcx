...
var currStreetData = [];
var regionArr = [];
var getRequestUrl = app.getRequestUrl;
...
Page({
    ...
    loadAreaData:function(){
      var that = this;
      wx.request({
        url: getRequestUrl + "/common/RegionAPI/GetSubRegion",
        data:{parent:0},
        success(result){
          that.setData({
    ...
    inputDetailAddressTap: function(e) {
      var detailAddressTap = e.detail.value;
      var that = this;
      that.data.detailAddress = detailAddressTap;
    },
    ...
    distpickerSure: function() {
      var that = this;
      var fullAddress = that.data.provinceArray[p].Name + " " + that.data.cityArray[c].Name + " " + (that.data.districtArray[d] == undefined ? '' : that.data.districtArray[d].Name) + (that.data.streetArray[s] == undefined ? '' : that.data.streetArray[s].Name);
      var regionId;
      var selcityname;
      if (that.data.streetArray[s] == '') {
        regionId = that.data.streetArray[s].Id;
        selcityname = that.data.streetArray[s].Id.Name;
      } else if (that.data.districtArray[d] == '') {
        regionId = that.data.districtArray[d].Id;
        selcityname = that.data.districtArray[d].Name;
      } else if (that.data.cityArray[c] == '') {
        regionId = that.data.cityArray[c].Id;
        selcityname = that.data.cityArray[c].Name;
      } else {
        regionId = that.data.provinceArray[p].Id;
        selcityname = that.data.provinceArray[p].Name;
      }
      var cssval = that.data.isCss;
    ...
    setAreaData: function(p, c, d, s) {
      ...
      var provinceIndex = 0,
       ...
       districtIndex = 0,
       streetIndex = 0;
      provinceIndex = that.data.provinceArray[p].Id;
      wx.request({
        url: getRequestUrl + "/common/RegionAPI/GetSubRegion",
        data: { parent: provinceIndex},
        success(cResult){
          that.setData({
            cityArray: cResult.data
          })
          cityIndex = that.data.cityArray[c].Id;
          wx.request({
            url: getRequestUrl + "/common/RegionAPI/GetSubRegion",
            data: { parent: cityIndex },
            success(dResult) {
              that.setData({
                districtArray: dResult.data
              })
              if (that.data.districtArray.length > 0){
                districtIndex = that.data.districtArray[d].Id;
                wx.request({
                  url: getRequestUrl + "/common/RegionAPI/GetSubRegion",
                  data: { parent: districtIndex },
                  success(sResult) {
                    that.setData({
                      streetArray: sResult.data
                    })
                  }
                })
              }else{
      ...
      console.log(name)
        ...
        app.getOpenId(function(openId) {
            var parameters = {
                ...
                // fromLatLng:"28.341472, 112.961834"
            }
    ...
    //       isDelete: false,
    //       detailAddress: name,
    //       FullRegionName: that.data.FullRegionName,
    //       regionId: that.data.regionId,
    //       lat: "28.341472",
    //       lng: "112.961834",
    //     })
