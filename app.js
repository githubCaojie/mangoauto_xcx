...
App({
    ...
    onLaunch: function (options) {
      mta.App.init({
        ...
        // 开启自动上报
        "autoReport": true,
        // 使用分析-下拉刷新次数/人数，必须先开通自定义事件，并配置了合法的
        "statPullDownFresh": true ,
        // 使用分析-分享次数/人数，必须先开通自定义事件，并配置了合法的 eventID
        "statShareApp": true ,
        // 使用分析-页面触底次数/人数，必须先开通自定义事件，并配置了合法的
        ...
        if (that.getIsLogin() && that.globalData.openId) {
            ...
            wx.login({
                success: function(res) {
                    if (res.code) {
                        const code = res.code;
                      that.globalData.wxlogincode = code;
                        wx.getUserInfo({
                            success: function(wxuser) {
                                ...
                                wx.request({
                                    ...
                                    success: function(result) {
                                        result = result.data;
                                        mta.Data.userInfo = { 'open_id': result.data.openid, 'phone': wx.getStorageSync("userInfo").CellPhone };
                                        if (result.success) {
                                          wx.request({
    ...
    //getRequestUrl: 'http://10.13.17.234:8066',
    getRequestUrl: 'https://mall.mangoauto.com.cn/',
    getUrl(route) {
    //return `http://10.13.17.234:8066/smallprogapi/${route}`;
      return `https://mall.mangoauto.com.cn/smallprogapi/${route}`;
    },
    ...
    globalData: {
        appId: 'wxeb6e792e0ea70d62',
        secret: '5140b67b50cc7dea60f64b924b541e40',
        userInfo: null,
        openId: '',
        wxUserInfo: null,
        wxlogincode:'', 
        users: [], // openid 2019.09.24
        // QQMapKey: 'R4IBZ-EY26G-JK2QA-IVPQT-GTBPS-BBFYN',
        QQMapKey: 'GTUBZ-2WXKX-6KJ4O-T7BUO-F43O6-B4BLG',
        ...
        getAccessToken: "Login/GetAccessToken", //获取令牌
    }
})